<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysql_query("update pesanan SET nama='$nama',tanggal_pesan='$tanggal_pesan',tanggal_digunakan='$tanggal_digunakan',user_id='$user_id',alamat='$alamat',telephone='$telephone',status='$status' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("pesanan.php"); }
		}
	}
			...
			if(!empty($_GET)){ 
				...
				?>
					...
					<form action="pesanan.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<input type="text" class="form-control" name="status" value="<?php echo $data->status; ?>" required><br>
